...
def _format_ejr_result(violation: EJRViolationResult, elapsed_time: float) -> dict:
    return {
        ...
        "number_of_unsat_voters_in_violation": (
            len(violation.unsat_voter_violation_union)
            if violation.unsat_voter_violation_union is not None
            else None
        ),
    ...
    for algo_name, ws_data in winning_sets_cache.items():
        ...
        result["results"][algo_name] = {
            "algo_stats": {
                ...
                "is_not_exhaustive": any(
                    costs[p] <= budget - cost_total
